...
from Model.hgsr_small import HGSR
from loss import GW_loss
...
class ZSSR:
    ...
    def __init__(self, input_img, conf=Config(), ground_truth=None, kernels=None):
        ...
        w, h, c = (self.input).shape
        w = (w//16)*16
        h = (h//16)*16
        self.input = input_img[:w, :h, :] if type(input_img) is not str else img.imread(input_img)[:w, :h, :]
        self.Y = False
        if len(self.input)==2:
            self.Y = True
        #input is ndarray
    ...
    def forward_backward_pass(self, lr_son, hr_father,criterion,optimizer):
        ...
        loss = GW_loss(hr_father,train_output)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    def quick_test(self):
        ...
        self.mse = (self.mse + GW_loss(self.gt_per_sf, self.sr) if self.gt_per_sf is not None else None)
        ...
        inp_img = (torch.Tensor(self.input).permute(2,0,1)).unsqueeze(0)
        rec_img = (torch.Tensor(self.reconstruct_output).permute(2,0,1)).unsqueeze(0)
        self.mse_rec.append(GW_loss(inp_img, rec_img))
        ...
        #                    if self.gt_per_sf is not None else None)
        self.interp_mse = (self.interp_mse + GW_loss(self.gt_per_sf, interp_sr) if self.gt_per_sf is not None else None)
        ...
        interp_rec_img = (torch.Tensor(interp_rec).permute(2,0,1)).unsqueeze(0)
        self.interp_rec_mse.append(GW_loss(inp_img, interp_rec_img))
